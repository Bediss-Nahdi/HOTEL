...
public class MyUserDetailsService implements UserDetailsService {
	...
	// conséquence
	@Autowired
	AdminService adminService; // PK? ==> pour nle findByUsername :p

	@Override
	public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
		// 4 // Je dois aller (logiquement) chercher le username et par voie de
		// conséquence le role
		//Admin user = adminService.findUserByUsername(username);
		AdminDTO user = adminService.findByUsername(username);
